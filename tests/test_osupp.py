...
from osupp.performance import calculate_osu_performance, Performance
import orjson
...
def test():
    ...
    # 进行 3 次计算
    perf1_attr = calculator.send(Performance(combo=706, misses=2, mehs=4, oks=34, large_tick_misses=0, slider_tail_misses=7))
    perf2_attr = calculator.send(Performance(combo=706, misses=2, mehs=4, oks=34, large_tick_hits=57, slider_tail_hits=485))
    perf_max_attr = calculator.send(Performance())
    # 分别校验绩效结果
    perf_result_obj = orjson.loads(perf_result)
